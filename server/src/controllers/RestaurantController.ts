...
export default class RestaurantController implements Crud {
    ...
    async create(request: Request, response: Response){
        const {name, street, food} = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(name, street, food);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = { name, street, food };
    ...
    async update(request: Request, response: Response){
        ...
        const isAnyUndefined = Citi.areValuesUndefined(name, street, food, id);
        if(isAnyUndefined) return response.status(400).send();

        const restaurantWithUpdatedValues = { name, street, food };
